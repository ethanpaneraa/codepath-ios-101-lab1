...
class ViewController: UIViewController,
  ...
  @IBAction func didDoubleTapKart(_ sender: UITapGestureRecognizer) {
    ...
    // Tip: Use the `translate` function below
      ...
      }// this moves the kart forward by the width of the containing view
  ...
  private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
      let currLocation = gestureRecognizer.location(in: view) // this gets the location of the gesture
      let kartView = gestureRecognizer.view // this gets a reference to the kart
      kartView?.center = currLocation // this moves the kart of that location
  }
  ...
  private func resetKarts() {
      UIView.animate(withDuration: 0.4) {
          // reset the state of the kart to be its default state
